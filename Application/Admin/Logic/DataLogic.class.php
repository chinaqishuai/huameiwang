...
class DataLogic extends CommonLogic
{
    ...
    public function restore($filePrefix)
    {
        ...
        $backupConfig = C('BACKUP');
        // 得到需要导入的文件
        $restoreFiles = isset($_SESSION['restore_cache']) ? $_SESSION['restore_cache']['files'] : $this->getRestoreFiles($filePrefix);
        ...
        foreach ($restoreFiles as $key => $storeFile) {
            ...
            while (!feof($fp)) {
                ...
                if ($exexuted >= 500) {
                    ...
                    // 已导入行数
                    $imported = isset($_SESSION['restore_cache']['imported']) ? $_SESSION['restore_cache']['imported'] : 0;
                    $imported += $exexuted;
                    $_SESSION['restore_cache']['imported'] = $imported;
                    fclose($fp);

                    return $this->resultReturn(self::EXECUTE_NOT_FINISH, array('imported' => $imported));
                }
            }
            fclose($file);
            // 删除已导入完成的文件
        ...
        return $this->resultReturn(self::EXECUTE_FINISH, array('time' => $time));
    }
    ...
    public function getRestoreFiles($filePrefix)
    {
        ...
        $files = array();
        while ($file = readdir($dirHandle)) {
            if (preg_match('/\.sql$/i', $file) && preg_match('/' . $filePrefix . '/i', $file)) {
                $files[] = $file;
            }
        }
        closedir($dirHandle);
        ...
        foreach ($files as $file) {
            // 取出分卷号作为键值
            $key = str_replace('.sql', '', str_replace($filePrefix . '_', '', $file));
            $sqlFiles[$key] = $file;
        }
    ...
    public function unzipFiles($zipFiles)
    {
        ...
        unzip($backupConfig['BACKUP_ZIP_DIR_PATH'] . $zipFile, $backupConfig['BACKUP_DIR_PATH']);

        if (count($zipFiles) > 0) {
            $_SESSION['unzip']['files'] = $zipFiles;
            return $this->resultReturn(self::EXECUTE_NOT_FINISH, array('file' => $zipFile));
        }
    ...
    {
        if (empty($sql) || '#' == $sql[0] || ('-' == $sql[0] && '-' == $sql[1])) {
            return false;
        }
