...
class FieldService extends CommonService
{
    ...
    public function add($field)
    {
        ...
        // 添加字段
        $ac = $Field->addColumn($model['tbl_name'], $field['name'], $field['type'], $field['length'], $field['value'], $field['comment']);
        ...
        if ('' == $field['value'] && isset($field['is_unique']) && 1 == $field['is_unique']) {
            $au = $Field->addUnique($model['tbl_name'], $field['name'], $uniqn);
        }

        if (false === $status || false === $ac || false === $ai || false === $au) {
            // 删除插入数据
    ...
    public function update($field)
    {
        ...
        // name
        if ($field['name'] != $old['name'] || $field['type'] != $old['type'] || $field['length'] != $old['length']) {
            $Field->alterColumnAttr($model['tbl_name'], $old['name'], $field['name'], $field['type'], $field['length'], $field['comment']);
        }

        // value
        if ($field['value'] != $old['value']) {
            $Field->alterColumnValue($model['tbl_name'], $field['name'], $field['value']);
        }

        // 先删除索引，再进行添加
        $oidxn = self::INDEX_PREFIX . $old['name'];
        if ($field['is_index'] != $old['is_index'] && 0 == $field['is_index']) {
            $Field->dropIndex($model['tbl_name'], $oidxn);
        }

        $ouniqn = self::UNIQUE_PREFIX . $old['name'];
        if ($field['is_unique'] != $old['is_unique'] && 0 == $field['is_unique']) {
            $Field->dropIndex($model['tbl_name'], $ouniqn);
        }

        $idxn = self::INDEX_PREFIX . $field['name'];
        if ($field['is_index'] != $old['is_index'] && 1 == $field['is_index']) {
            $Field->dropIndex($model['tbl_name'], $oidxn);
            $Field->addIndex($model['tbl_name'], $field['name'], $idxn);
        }

        $uniqn = self::UNIQUE_PREFIX . $field['name'];
        if ($field['is_unique'] != $old['is_unique'] && 1 == $field['is_unique']) {
            $Field->addIndex($model['tbl_name'], $field['name'], $uniqn);
            $Field->dropIndex($model['tbl_name'], $ouniqn);
        }

        if (false === $status) {
            $Field->alterColumnAttr($model['tbl_name'], $field['name'], $old['name'], $old['type'], $old['length'], $old['comment']);
            $Field->alterColumnValue($model['tbl_name'], $old['name'], $old['value']);
            (1 == $old['is_index']) ?
                            $Field->addIndex($model['tbl_name'], $old['name'], $oidxn) : $Field->dropIndex($model['tbl_name'], $idxn);

            $ouniqn = self::UNIQUE_PREFIX . $old['name'];
            (1 == $old['is_unique']) ?
                            $Field->addUnique($model['tbl_name'], $old['name'], $ouniqn) : $Field->dropIndex($model['tbl_name'], $uniqn);
