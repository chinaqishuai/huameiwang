        ...
        foreach ($fields as $field) {
            ...
            if ($inputService->isFileInput($field['input']['type'])) {
                if (!$once) {
                    ...
                    if (false === $uploadInfo['status'] && !empty($uploadInfo['info'])) {
                        // 上传失败
                ...
                if (true === $uploadInfo['status'] && !$this->isEmpty($_FILES[$fn]['tmp_name']) && is_array($uploadInfo['info'][0])) {
                    // 处理真正上传过的file表单域
            ...
            if ('date' == $field['input']['type'] && !empty($data[$fn])) {
                $result = $this->checkTypeContraint($field['input']['type'], $data[$fn]);
                if (!$result['status']) {
                    $msg = "{$fm}日期格式不正确！";
                    return $this->errorResultReturn($msg);
                }
            ...
            // 字符长度
            if (('CHAR' == $field['type'] || 'VARCHAR' == $field['type']) && !empty($data[$fn]) && strlen($data[$fn]) > $field['length']) {
            ...
            // 字段必填
            if (1 != $field['is_system'] && 1 == $field['is_require'] && empty($field['auto_fill']) && (!isset($data[$fn]) || empty($data[$fn]))) {
                return $this->errorResultReturn("{$fm}必需填写！");
            }

            // 字段唯一
            if (1 != $field['is_system'] && 1 == $field['is_unique'] && !empty($data[$fn])) {
                $isUnique = $this->isRowUnique($ctrlName, $field['name'], $data[$fn], $data['id']);
                if (!$isUnique) {
            ...
            // 系统字段 auto_fill 自动填充
            if ($field['is_system'] && !empty($field['auto_fill']) && ('both' == $field['fill_time'] || $time == $field['fill_time'])) {
            ...
            if (!empty($field['auto_fill']) && empty($data[$fn]) && ('both' == $field['fill_time'] || $time == $field['fill_time'])) {
