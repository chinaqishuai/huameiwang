...
class ModelService extends CommonService
{
    ...
    public function add($model)
    {
        ...
        // 创建数据表
        $createTblStatus = $Model->createTable($model['tbl_name'], $model['has_pk'], $model['tbl_engine'], $model['description']);
        // 添加系统字段
        ...
        if (false === $addStatus || false === $createTblStatus || false === $addFieldStatus || false === $amns) {
            $Model->rollback();
    ...
    public function update($model)
    {
        ...
        if ($model['tbl_name'] != $old['tbl_name']) {
            $utnStatus = $Model->updateTableName($old['tbl_name'], $model['tbl_name']);
        }
        // 更新数据表注释
        if ($model['description'] != $old['description']) {
            $utcStatus = $Model->updateTableComment($model['tbl_name'], $model['description']);
        }
        // 更新菜单
        if (($model['menu_name'] != $old['menu_name'] || $model['tbl_name'] != $old['tbl_name']) && 0 == $old['is_inner']) {
            $this->replaceMenuItem($model, $old);
        }

        if (false === $updateStatus || false === $utnStatus || false === $utcStatus) {
            $Model->rollback();
            // 撤回更新
            if (0 == $old['is_inner']) {
