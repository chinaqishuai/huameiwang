...
class EmptyController extends CommonController
{
    ...
    public function index()
    {
        ...
        foreach ($fields as $field) {
            ...
            if (($field['is_system'] && $field['is_list_show']) && ('created_at' == $fn || 'updated_at' == $fn)) {
            ...
            if (in_array($field['input']['type'], $this->types) && !empty($field['input']['opt_value'])) {
                $opts = D('Input', 'Logic')
            ...
            // 关联表类型
            if (0 != $field['relation_model'] && !empty($field['relation_value']) && !empty($field['relation_field'])) {
                // 被关联的模型
    ...
    public function create()
    {
        ...
        return $this->successReturn('成功添加数据！', U(CONTROLLER_NAME . '/index'));
    }
    ...
    public function update()
    {
        ...
        if (!isset($_POST[$iname]['id']) || is_null(M(CONTROLLER_NAME)->getById($_POST[$iname]['id']))) {
        ...
        // 创建数据
        $data = $_POST[$iname];
        $result = $defaultService->create($data, $fields, CONTROLLER_NAME, 'update');
        if (!$result['status']) {
            return $this->errorReturn($result['data']['error']);
        }
        ...
        return $this->successReturn('更新数据成功！', U(CONTROLLER_NAME . '/index'));
    }
    ...
    {
        if (!isset($_GET['id']) || is_null(M(CONTROLLER_NAME)->getById($_GET['id']))) {
