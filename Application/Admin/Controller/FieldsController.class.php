...
class FieldsController extends CommonController
{
    ...
    {
        $result = D('Field', 'Service')->checkFieldName($_GET['field_name'], $_GET['model_id'], $_GET['field_id']);
        if ($result['status']) {
    ...
    public function checkFieldLabel()
    {
        $result = D('Field', 'Service')
                ->checkFieldComment($_GET['field_label'], $_GET['model_id'], $_GET['field_id']);
        if ($result['status']) {
    ...
    public function delete()
    {
        ...
        if (!D('Model', 'Service')->hasField($_GET['model_id'], $_GET['field_id'])) {
            return $this->errorReturn('无效的操作！');
        }
