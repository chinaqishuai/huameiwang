...
class ModelsController extends CommonController
{
    ...
    public function show()
    {
        ...
        foreach ($model['fields'] as $key => $field) {
            $input = M('Input')->field('show_order')
                    ...
                    ->find();
            $model['fields'][$key]['show_order'] = (is_null($input) || 0 == $input['show_order']) ? 0 : $input['show_order'];
    ...
    {
        $result = D('Model', 'Service')->checkModelName($_GET['model_name'], $_GET['id']);
        if ($result['status']) {
    ...
    {
        $result = D('Model', 'Service')->checkMenuName($_GET['menu_name'], $_GET['id']);
        if ($result['status']) {
