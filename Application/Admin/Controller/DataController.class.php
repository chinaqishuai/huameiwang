...
class DataController extends CommonController
{
    ...
    public function downloadFile()
    {
        ...
        if (empty($_GET['file_name']) || empty($_GET['file_type']) || !in_array($fileType, array('zip', 'sql'))) {
            $this->error($info);
        }

        $backupConfig = C('BACKUP');
        $backupDir = ('zip' !== $fileType) ? $backupConfig['BACKUP_DIR_PATH'] : $backupConfig['BACKUP_ZIP_DIR_PATH'];
    ...
    public function deleteSqlFiles()
    {
        ...
        $this->successReturn('已删除：' . implode("、", $_POST['sql_files']), U('Data/restore', array('time' => time())));
    }
    ...
    public function zipSqlFiles()
    {
        ...
        if (false === zip($sqlFiles, $backupConfig['BACKUP_DIR_PATH'], $backupConfig['BACKUP_ZIP_DIR_PATH'], $zipName)) {
    ...
    {
        if (!IS_POST || !isset($_POST['action']) || !in_array($_POST['action'], array('optimize', 'repair'))) {
            $this->errorReturn('无效的操作！');
        }
