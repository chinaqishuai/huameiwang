...
{
    if (isset($action_name) && (false !== strpos($controller_name, CONTROLLER_NAME)) && ACTION_NAME == $action_name) {
        return $style;
    }

    if (!isset($action_name) && (false !== strpos($controller_name, CONTROLLER_NAME))) {
        return $style;
    }
...
function genRadios($name, array $list, $checked = 0, $class = '')
{
    ...
    $i = 0;
    foreach ($list as $text => $value) {
        $html .= $i == $checked ? genRadio($name, $text, $value, true, $class) : genRadio($name, $text, $value);
        $i++;
    }
...
function genCheckboxs($name, array $list, $checked, $class = '')
{
    ...
    foreach ($list as $text => $value) {
        $html .= in_array($i, $checked) ?
                genCheckbox($name, $text, $value, true, $class) : genCheckbox($name, $text, $value);
        $i++;
    }
